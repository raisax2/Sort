...
{
  if (getPointerTo(i) != nullptr && getPointerTo(j) != nullptr)
  {
    T swapping = (getPointerTo(j))->getItem();//temperary hold
    getPointerTo(j)->setItem(getPointerTo(i)->getItem()); //getting j setting to i's item
    getPointerTo(i)->setItem(swapping); // setting i to temp
  }
}
